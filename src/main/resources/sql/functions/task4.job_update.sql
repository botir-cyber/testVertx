DROP FUNCTION IF EXISTS task4.job_update;
CREATE FUNCTION task4.job_update(
  in_login_id BIGINT,
  in_job_id BIGINT,
  in_job_name TEXT)
RETURNS BIGINT
AS $$
/******************************************************************************
**		File: task4.job_update.sql
**		Name: task4.job_update
...
DECLARE
  FN_NAME CONSTANT TEXT := 'task4.job_update';
  STEP_INDEX INTEGER;
...
-- to do need to complete this step
  IF NOT EXISTS(SELECT * FROM task4.job WHERE job.job_id = in_job_id AND NOT deleted) THEN
